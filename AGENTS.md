...
The camera mimics a Crossy Road-style orthographic rig: arrow keys pan across the pasture plane, and the scroll wheel adjusts altitude (scroll up = closer, scroll down = taller view). The left “Camera Settings” tab writes to `localStorage`, feeding `cameraSettings` (offsets, limits, pan speed, frustum). Saving in the UI re-runs `applyCameraSettings()` and updates the orthographic frustum without a reload. Collectible nodes live in the `nodes` array-extend their `userData` for new mechanics (power boosts, decay timers). Keep score logic centralized in `collectNode` so HUD values stay in sync.
