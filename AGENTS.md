...
The camera is diagonal and user-controlled: arrow keys pan the view, and the mouse scroll wheel adjusts altitude (scroll up lowers the camera, scroll down raises it). The Camera Settings tab persists offsets/limits/pan speed via `localStorage`; saving updates `cameraSettings` and reruns `applyCameraSettings()` so the perspective rig reacts immediately. Collectible nodes live in the `nodes` array-extend their `userData` objects for new mechanics (power boosts, decay timers). Keep score logic centralized in `collectNode` so HUD values stay in sync.
