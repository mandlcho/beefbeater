...
The camera mimics a Crossy Road-style orthographic rig: arrow keys pan across the pasture plane, and the scroll wheel adjusts altitude (scroll up = closer, scroll down = taller view). Tune `cameraState` and `handleCameraPan` for different bounds or easing. Collectible nodes live in the `nodes` array—extend their `userData` for new mechanics (power boosts, decay timers). Keep score logic centralized in `collectNode` so HUD values stay in sync.
